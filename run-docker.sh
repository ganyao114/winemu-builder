...
if [ -n "$(command -v getenforce)" ] && [ "$(getenforce)" = Enforcing ]; then
	VOLUME=$REPOROOT:$CONTAINER_HOME_DIR/winemu-build:z
else
	VOLUME=$REPOROOT:$CONTAINER_HOME_DIR/winemu-build
fi

: ${TERMUX_BUILDER_IMAGE_NAME:=ghcr.io/ganyao114/winemu-builder}
: ${CONTAINER_NAME:=winemu-builder}
...
. "$BUILDER_SCRIPTDIR/docker.sh"; docker__setup_docker_exec_traps
